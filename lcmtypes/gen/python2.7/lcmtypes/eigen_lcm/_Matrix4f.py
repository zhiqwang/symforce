...
class Matrix4f(object):
    ...
    def __init__(
        ...
        if _skip_initialize:
            return
        self.data = [ 0.0 for dim0 in range(16) ] if data is None else data  # type: T.List[float]
    ...
    def encode(self):
        # type: () -> bytes
        buf = BytesIO()
        buf.write(Matrix4f._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        # type: (T.BinaryIO) -> None
        buf.write(Matrix4f._CACHED_STRUCT_0.pack(*self.data[:16]))

    @staticmethod
    def decode(data):
        # type: (T.Union[bytes, T.BinaryIO]) -> Matrix4f
        ...
        if buf.read(8) != Matrix4f._get_packed_fingerprint():
            raise ValueError("Decode error")
        return Matrix4f._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        # type: (T.BinaryIO) -> Matrix4f
        self = Matrix4f(_skip_initialize=True)
        self.data = list(Matrix4f._CACHED_STRUCT_0.unpack(buf.read(64)))
        return self

    @staticmethod
    def _get_hash_recursive(parents):
        # type: (T.List[T.Type]) -> int
        if Matrix4f in parents: return 0
        tmphash = (0x782a36eaf52557ee) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        # type: () -> bytes
        if Matrix4f._packed_fingerprint is None:
            Matrix4f._packed_fingerprint = struct.pack(">Q", Matrix4f._get_hash_recursive([]))
        return Matrix4f._packed_fingerprint
    ...
    def deepcopy(self, **kwargs):
        ...
        for key in kwargs:
            if not hasattr(result, key):
                raise KeyError("Type Matrix4f does not have attribute: " + str(key))
            setattr(result, key, kwargs[key])
        return result

    _CACHED_STRUCT_0 = struct.Struct(">16f")
