...
class Matrix3d(object):
    ...
    def encode(self):
        # type: () -> bytes
        buf = BytesIO()
        buf.write(Matrix3d._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        # type: (T.BinaryIO) -> None
        buf.write(Matrix3d._CACHED_STRUCT_0.pack(*self.data[:9]))

    @staticmethod
    def decode(data):
        # type: (T.Union[bytes, T.BinaryIO]) -> Matrix3d
        ...
        if buf.read(8) != Matrix3d._get_packed_fingerprint():
            raise ValueError("Decode error")
        return Matrix3d._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        # type: (T.BinaryIO) -> Matrix3d
        self = Matrix3d(_skip_initialize=True)
        self.data = list(Matrix3d._CACHED_STRUCT_0.unpack(buf.read(72)))
        return self

    @staticmethod
    def _get_hash_recursive(parents):
        # type: (T.List[T.Type]) -> int
        if Matrix3d in parents: return 0
        tmphash = (0x7e2b2f4ef42123f7) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        # type: () -> bytes
        if Matrix3d._packed_fingerprint is None:
            Matrix3d._packed_fingerprint = struct.pack(">Q", Matrix3d._get_hash_recursive([]))
        return Matrix3d._packed_fingerprint
    ...
    def deepcopy(self, **kwargs):
        ...
        for key in kwargs:
            if not hasattr(result, key):
                raise KeyError("Type Matrix3d does not have attribute: " + str(key))
            setattr(result, key, kwargs[key])
        return result

    _CACHED_STRUCT_0 = struct.Struct(">9d")
