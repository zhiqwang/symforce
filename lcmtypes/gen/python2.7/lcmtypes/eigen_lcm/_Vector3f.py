...
class Vector3f(object):
    ...
    def encode(self):
        # type: () -> bytes
        buf = BytesIO()
        buf.write(Vector3f._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        # type: (T.BinaryIO) -> None
        buf.write(Vector3f._CACHED_STRUCT_0.pack(*self.data[:3]))

    @staticmethod
    def decode(data):
        # type: (T.Union[bytes, T.BinaryIO]) -> Vector3f
        ...
        if buf.read(8) != Vector3f._get_packed_fingerprint():
            raise ValueError("Decode error")
        return Vector3f._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        # type: (T.BinaryIO) -> Vector3f
        self = Vector3f(_skip_initialize=True)
        self.data = list(Vector3f._CACHED_STRUCT_0.unpack(buf.read(12)))
        return self

    @staticmethod
    def _get_hash_recursive(parents):
        # type: (T.List[T.Type]) -> int
        if Vector3f in parents: return 0
    ...
    def _get_packed_fingerprint():
        # type: () -> bytes
        if Vector3f._packed_fingerprint is None:
            Vector3f._packed_fingerprint = struct.pack(">Q", Vector3f._get_hash_recursive([]))
        return Vector3f._packed_fingerprint
    ...
    def deepcopy(self, **kwargs):
        ...
        for key in kwargs:
            if not hasattr(result, key):
                raise KeyError("Type Vector3f does not have attribute: " + str(key))
