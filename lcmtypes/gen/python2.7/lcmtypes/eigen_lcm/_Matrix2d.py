...
class Matrix2d(object):
    ...
    def encode(self):
        # type: () -> bytes
        buf = BytesIO()
        buf.write(Matrix2d._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        # type: (T.BinaryIO) -> None
        buf.write(Matrix2d._CACHED_STRUCT_0.pack(*self.data[:4]))

    @staticmethod
    def decode(data):
        # type: (T.Union[bytes, T.BinaryIO]) -> Matrix2d
        ...
        if buf.read(8) != Matrix2d._get_packed_fingerprint():
            raise ValueError("Decode error")
        return Matrix2d._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        # type: (T.BinaryIO) -> Matrix2d
        self = Matrix2d(_skip_initialize=True)
        self.data = list(Matrix2d._CACHED_STRUCT_0.unpack(buf.read(32)))
        return self

    @staticmethod
    def _get_hash_recursive(parents):
        # type: (T.List[T.Type]) -> int
        if Matrix2d in parents: return 0
    ...
    def _get_packed_fingerprint():
        # type: () -> bytes
        if Matrix2d._packed_fingerprint is None:
            Matrix2d._packed_fingerprint = struct.pack(">Q", Matrix2d._get_hash_recursive([]))
        return Matrix2d._packed_fingerprint
    ...
    def deepcopy(self, **kwargs):
        ...
        for key in kwargs:
            if not hasattr(result, key):
                raise KeyError("Type Matrix2d does not have attribute: " + str(key))
