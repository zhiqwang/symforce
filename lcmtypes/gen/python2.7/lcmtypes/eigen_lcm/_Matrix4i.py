...
class Matrix4i(object):
    ...
    def __init__(
        ...
        if _skip_initialize:
            return
        self.data = [ 0 for dim0 in range(16) ] if data is None else data  # type: T.List[int]
    ...
    def _encode_one(self, buf):
        ...
        buf.write(Matrix4i._CACHED_STRUCT_0.pack(*self.data[:16]))

    @staticmethod
    def decode(data):
        # type: (T.Union[bytes, T.BinaryIO]) -> Matrix4i
    ...
    @staticmethod
    def _decode_one(buf):
        # type: (T.BinaryIO) -> Matrix4i
        self = Matrix4i(_skip_initialize=True)
        self.data = list(Matrix4i._CACHED_STRUCT_0.unpack(buf.read(64)))
        return self
    ...
    def deepcopy(self, **kwargs):
        ...
        for key in kwargs:
            if not hasattr(result, key):
                raise KeyError("Type Matrix4i does not have attribute: " + str(key))
            setattr(result, key, kwargs[key])
        return result

    _CACHED_STRUCT_0 = struct.Struct(">16i")
