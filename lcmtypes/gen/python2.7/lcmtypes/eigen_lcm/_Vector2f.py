...
class Vector2f(object):
    ...
    def encode(self):
        # type: () -> bytes
        buf = BytesIO()
        buf.write(Vector2f._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        # type: (T.BinaryIO) -> None
        buf.write(Vector2f._CACHED_STRUCT_0.pack(*self.data[:2]))

    @staticmethod
    def decode(data):
        # type: (T.Union[bytes, T.BinaryIO]) -> Vector2f
        ...
        if buf.read(8) != Vector2f._get_packed_fingerprint():
            raise ValueError("Decode error")
        return Vector2f._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        # type: (T.BinaryIO) -> Vector2f
        self = Vector2f(_skip_initialize=True)
        self.data = list(Vector2f._CACHED_STRUCT_0.unpack(buf.read(8)))
        return self

    @staticmethod
    def _get_hash_recursive(parents):
        # type: (T.List[T.Type]) -> int
        if Vector2f in parents: return 0
        tmphash = (0x5c782a36eaf52458) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        # type: () -> bytes
        if Vector2f._packed_fingerprint is None:
            Vector2f._packed_fingerprint = struct.pack(">Q", Vector2f._get_hash_recursive([]))
        return Vector2f._packed_fingerprint
    ...
    def deepcopy(self, **kwargs):
        ...
        for key in kwargs:
            if not hasattr(result, key):
                raise KeyError("Type Vector2f does not have attribute: " + str(key))
            setattr(result, key, kwargs[key])
        return result

    _CACHED_STRUCT_0 = struct.Struct(">2f")
