...
class Vector1d(object):
    ...
    def encode(self):
        # type: () -> bytes
        buf = BytesIO()
        buf.write(Vector1d._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        # type: (T.BinaryIO) -> None
        buf.write(Vector1d._CACHED_STRUCT_0.pack(*self.data[:1]))

    @staticmethod
    def decode(data):
        # type: (T.Union[bytes, T.BinaryIO]) -> Vector1d
        ...
        if buf.read(8) != Vector1d._get_packed_fingerprint():
            raise ValueError("Decode error")
        return Vector1d._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        # type: (T.BinaryIO) -> Vector1d
        self = Vector1d(_skip_initialize=True)
        self.data = list(Vector1d._CACHED_STRUCT_0.unpack(buf.read(8)))
        return self

    @staticmethod
    def _get_hash_recursive(parents):
        # type: (T.List[T.Type]) -> int
        if Vector1d in parents: return 0
        tmphash = (0x7e2b2f4ef42123ef) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        # type: () -> bytes
        if Vector1d._packed_fingerprint is None:
            Vector1d._packed_fingerprint = struct.pack(">Q", Vector1d._get_hash_recursive([]))
        return Vector1d._packed_fingerprint
    ...
    def deepcopy(self, **kwargs):
        ...
        for key in kwargs:
            if not hasattr(result, key):
                raise KeyError("Type Vector1d does not have attribute: " + str(key))
            setattr(result, key, kwargs[key])
        return result

    _CACHED_STRUCT_0 = struct.Struct(">1d")
